/**
 * Unit-Tests fuer Migration v11 (T3.1 Apollo-Pause).
 *
...
const MIGRATION_PATH = join(__dirname, '../../db-migration-v11-sequence-pause-log.sql');

function loadMigration(): string {
  return readFileSync(MIGRATION_PATH, 'utf8');
}

test('v11: legt sequence_pause_log-Tabelle mit Pflicht-Spalten an', () => {
...
test('v11: hat Indexes auf lead_id, sequence_type, paused_at', () => {
  const sql = loadMigration();
...
test('v11: Audit-Snapshot-INSERT erfolgt VOR UPDATE', () => {
  const sql = loadMigration();
...
test('v11: Pause-Scope ist auf bauunternehmen und handwerk beschraenkt', () => {
  const sql = loadMigration();
  assert.match(
    sql,
...
test('v11: DOWN-Migration-Kommentare vorhanden und reversibel', () => {
  const sql = loadMigration();
...
test('v11: operator-Tag identifiziert Track-3-Quelle', () => {
  const sql = loadMigration();
  assert.match(sql, /'track-3-migration-v11'/);
});
