...
export async function GET(request: NextRequest) {
  ...
  if (await isAlreadyRunning('partner_supervisor', 8)) {
    console.log('[partner-supervisor] Bereits aktiv - ueberspringe diesen Lauf.');
    return NextResponse.json({ ok: true, skipped: true, reason: 'already_running' });
  }

  const runId = crypto.randomUUID();
  const startTime = Date.now();
  console.log(`[partner-supervisor] Starte run ${runId} (max 20 Iterationen, 300s Budget)...`);

  // Write started log BEFORE calling Gemini so the run is always visible in the dashboard
  await writeStartLog(runId, 'partner_supervisor');
  ...
  try {
    ...
    await writeEndLog(runId, 'partner_supervisor', result.success ? 'completed' : 'partial', {
    ...
    await writeEndLog(runId, 'partner_supervisor', 'error', { error: String(err) });
