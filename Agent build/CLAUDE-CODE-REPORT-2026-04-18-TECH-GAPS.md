...
## Addendum 2026-04-18 (spaet) — ntfy.sh als zweite Push-Spur

Folge-PR zu T3 nach Angies Feedback "kein Slack, aber ntfy-iOS-App funktioniert".

**Was rein:**
- `lib/observability/logger.ts`: neue `notifyNtfy(entry)` analog zu `notifySlack`, POST an `NTFY_TOPIC_URL` mit Title-/Priority-Headers (Title `[<agent>] <level>`, Priority `default` oder `high` bei `context.critical === true`)
- `observe.error` ruft beide Channels parallel via `Promise.allSettled`. Ein ausfallender Channel blockt den anderen nicht.
- 8 neue Tests (17 total in observability.test.ts) inkl. "one channel failing does not block the other"
- `.env.example` aktualisiert mit `SLACK_ALERT_WEBHOOK=` und `NTFY_TOPIC_URL=https://ntfy.sh/praxisnovaai-alerts-task110`
- Runbook A umbenannt auf "Slack-Alert ODER ntfy-Push" mit Format-Beschreibung fuer beide Kanaele und "fehlender Kanal als Signal" Diagnose

**Merge-Reihenfolge:** Dieser PR haengt von T3 (PR #25) ab. Wenn T3 zuerst gemerged wird, rebasiert sich der ntfy-PR sauber auf main.

**ENV-Aktion offen:** `NTFY_TOPIC_URL=https://ntfy.sh/praxisnovaai-alerts-task110` in Vercel (Production + Preview) setzen. Push-Test bereits bestanden.

---

**Status:** Tech-Gaps Wave 1 abgeschlossen. 4 PRs + ntfy-Follow-up mergebar sobald Angie reviewed.
**PR-URLs:**
...
- Docs: https://github.com/hertleanjuli-afk/praxisnova-sales-control/pull/27
- ntfy-integration (dieser): wird unten generiert
