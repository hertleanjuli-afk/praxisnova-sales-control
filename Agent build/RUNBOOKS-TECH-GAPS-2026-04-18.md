...
## Runbook A — Slack-Alert ODER ntfy-Push ist angekommen, was tun?

**Kontext:** T3 hat `observe.error()` ausgeloest und sendet parallel an beide Kanaele (`SLACK_ALERT_WEBHOOK` und `NTFY_TOPIC_URL`, falls gesetzt). Ein Ausfall eines Kanals blockt den anderen nicht (Promise.allSettled).

**Slack-Format (falls SLACK_ALERT_WEBHOOK gesetzt):**
...
**ntfy-Format (iOS-App Push, falls NTFY_TOPIC_URL gesetzt):**

- Title-Header: `[<agent_name>] <level>` (z.B. `[outreach_strategist] error`)
- Priority: `default` (normal) oder `high` wenn `context.critical === true` (Sound + Banner)
- Body: message + skill-Zeile + JSON-Context (bis 800 Zeichen)

**Dasselbe Runbook gilt fuer beide Kanaele.** Unterschied ist nur wie du den Alert empfangen hast. Angie nutzt aktuell ntfy-iOS-App (Topic: `praxisnovaai-alerts-task110`). Slack ist vorbereitet aber nicht aktiv bei Angie. Wenn beide Kanaele gleichzeitig ankommen: das ist gesund, bedeutet beide Push-Channels funktionieren.

**Fehlender Kanal als eigenes Signal:**
- Slack-Alert kommt, ntfy nicht -> ntfy-Topic-URL pruefen oder iOS-Subscription re-subscriben
- ntfy-Push kommt, Slack nicht -> Slack-Webhook pruefen (vermutlich 404/expired)
- Weder Slack noch ntfy, aber `error_logs` enthaelt Eintraege -> beide ENVs in Vercel gesetzt?
